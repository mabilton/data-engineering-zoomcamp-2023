...
def extract_from_gcs(
    ...
    gcs_path = f"{gcs_dir}/{color}/{file_name}"
    gcs_block = GcsBucket.load(bucket_block)
    local_save_dir = os.path.join(save_dir, color)
    if not os.path.isdir(local_save_dir):
        os.makedirs(local_save_dir)
    gcs_block.get_directory(from_path=gcs_path, local_path=local_save_dir)
    return Path(local_save_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=100_000,
        help=(
            "Size of chunks to use when uploading data to BigQuery (default: 100_000)."
        ),
        type=int,
    )
